...
New work is admission-controlled. Every task reserves one attempt of what it should cost: executor `e`, decomposer `α + β·k`, root `α + β·k + σ`. A decomposer that hasn't spawned yet also reserves its cheapest split (one child). The root's bootstrap, each SPAWN batch, each TASK TOO LARGE conversion and each respawn is admitted only while spent plus all outstanding reservations stays within the budget minus the death floor. A batch that doesn't fit is cut to the largest k that does, and the decomposer is told what was dropped. Each task is stopped once its agents together spend one attempt's cost for every agent the attempt cap allows (`4 × its own attempt cost`, a decomposer priced at its full fan-out, plus any executor spend a conversion carried in): 108 for an executor, 130 for a decomposer, 195 for the root. The figures and their evaluation are in `sims/reservation_formula/REPORT.md`.
